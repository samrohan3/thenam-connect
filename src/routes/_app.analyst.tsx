...
import { 
  FileBarChart, ExternalLink, Image, LayoutTemplate, Plus, MoreVertical, 
...
export const Route = createFileRoute("/_app/analyst")({
  head: () => ({ meta: [{ title: "Analyst Workspace — Thenam ERP" }] }),
...
function getIconForType(type: string) {
  switch (type?.toLowerCase()) {
    ...
    case 'figma': return <FileBarChart className="w-4 h-4 text-cyan-500" />;
    case 'google sheets':
...
function AnalystPage() {
  ...
  const { data: links, isLoading } = useWorkspaceLinks({ workspace: "analyst" });
  const { data: recentLinks } = useRecentWorkspaceLinks("analyst");
  ...
  const openEdit = (link: any) => {
    ...
    setCategory(link.category || "Analytics");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload = {
      name, description, url, type, category, visibility, workspace: "analyst"
    };
  ...
  return (
    <div className="p-8 space-y-6 max-w-6xl mx-auto">
      <div className="flex items-center gap-3 border-b border-border pb-4">
        ...
        <div>
          <h1 className="text-2xl font-bold">Analyst Workspace</h1>
          <p className="text-sm text-muted-foreground">Strategic planning, external spreadsheets, and meetings.</p>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="p-6 rounded-2xl border bg-card/50 space-y-4">
          ...
          <p className="text-sm text-muted-foreground">Access your external analysis tools and meetings.</p>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-md rounded-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingId ? "Update details for this analyst link." : "Add a new external link to the Analyst Workspace."}
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 pt-2">
            <div className="space-y-1.5">
              ...
              <Input 
                ...
                placeholder="e.g., Q3 Strategy Deck" 
